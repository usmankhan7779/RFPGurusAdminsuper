...
export class ViewpromoComponent implements OnInit {
  ...
  postoffer(page) {
    ...
    return this._serv.promocode_get_post(this.pageSize, moment(this.model.datefrom).format('YYYY-MM-DD'), moment(this.model.dateto).format('YYYY-MM-DD'), this.model.code, this.model.active,this.model.subscription, page).subscribe(
      // moment(this.date.value['dateto']).format('YYYY-MM-DD'),
      data => {
